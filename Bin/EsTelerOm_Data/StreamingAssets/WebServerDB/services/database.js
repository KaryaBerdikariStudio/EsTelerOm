...
// —————————————————————————————————————————————————————————————
// Log for sanity:
console.log("▶️ [DB] process.cwd() is:", process.cwd());

// Build path relative to your launch directory:
const dataDir = process.cwd();
const dbPath  = path.join(dataDir, "game_database.db");
console.log("▶️ [DB] Using dbPath:", dbPath);

// (Optional) create folder if somehow missing:
if (!fs.existsSync(dataDir)) {
  console.log("🗂️ [DB] Creating dataDir:", dataDir);
...
function getClientStatusType(type) {
  return new Promise((res, rej) => {
    db.all(
      ...
      (err, rows) => {
        if (err) return rej(err);
        // rows is an array of { nama: "...", status: "..." }
        res(rows);
      }
    );
  });
  ...
  return new Promise((res, rej) => {
    db.all(
      `SELECT playerName, playerSkor, timestamp
         FROM Skor
        ORDER BY playerSkor DESC, timestamp DESC
         LIMIT 10`, // Limit to last 10 scores
      [],
      (err, rows) => {
        if (err) return rej(err);
        res(rows);
...
function insertLog({ tbl, events, message }) {
  const sql = `
    INSERT INTO Logs(tbl, events, message)
      VALUES(?,?,?)
  `;
  return new Promise((res, rej) => {
    db.run(sql, [tbl, events, JSON.stringify(message)], err => err ? rej(err) : res());
  });
}
...
function deleteLogs() {
  return new Promise((res, rej) => {
    db.run(`DELETE FROM Logs`, err => err ? rej(err) : res());
  });
}
